...
def buttonPress(last):
    ...
    if tempLast == BTN_RELEASED and buttons[0].value == False:
        nextState = BTN_PRESSED
        currentMode = (currentMode + 1) % COUNT_OF_MODES
        draw_screen("", 1, 1)

    # Tasten 2–6 = Funktion ausführen
    for i in range(1, 6):
        if tempLast == BTN_RELEASED and buttons[i].value == False:
            nextState = BTN_PRESSED
    ...
    if tempLast == BTN_PRESSED and not any(btn.value == False for btn in buttons):
        nextState = BTN_RELEASED
        draw_screen("", 1, 1)
